...
export const NetworkIcon = ({ network }: Profile) => {
  switch (network) {
    ...
    case "Other":
      return <FaGlobe className="w-5 h-5 themed-icon" />;
    default:
      return null;
  }
};
...
export const NetworkProfiles = ({
  profiles,
  showNetworks,
  className = "inline-flex rounded-full mr-6",
}: NetworkProfileProps) => {
  ...
  return (
    <Fragment>
      {profiles?.map((profile, index) => (
        <Link
          className={`hover:text-cyan-500 bg-transparent hover:border-0 ${className}`}
          href={profile.url ?? ""}
          target="_blank"
