...
const fallbackData = INSTAGRAM_DATA.data?.filter(
  (instagramObj: InstagramMedia) => instagramObj.caption?.includes("#random"),
);
  ...
  try {
    if (isDev()) {
      res
        .status(200)
        .json(
          INSTAGRAM_DATA.data?.filter((instagramObj: InstagramMedia) =>
    ...
    return res
      .status(200)
      .json(
        instagramObj?.data?.filter((instagramObj: InstagramMedia) =>
          instagramObj.caption?.includes("#random"),
        ),
      );
  } catch (e) {
